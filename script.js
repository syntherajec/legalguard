...
// Try one API call with a specific key and model, supports AbortController
...
// Race all (key × model) combinations in parallel — first valid result wins
async function raceAllRequests(apiKeys, contract) {
  const controllers = [];

  // Build one promise per (key, model) pair that resolves on success, rejects on failure
  const promises = [];
    ...
    for (const model of SMART_MODELS) {
      ...
      promises.push(
        tryRequest(apiKey, model, contract, ctrl.signal)
          .then(raw => {
            const result = parseJSON(raw); // throws if JSON invalid
            return result;                 // valid result — this promise resolves
          })
          // on any error (network, HTTP, bad JSON) just reject so race continues
      );
    }
  }

  // Promise that resolves with first successful result, aborts all others
    ...
    promises.forEach((p, idx) => {
      p.then(result => {
        ...
        if (rejectedCount === total) {
          reject(new Error('Semua model & key gagal. Periksa API key atau koneksi Anda.'));
        }
      });
    });
  });
...
async function analyzeContract() {
  ...
  try {
    ...
    if (loadingSection) loadingSection.classList.add('hidden');
    if (app) app.classList.remove('hidden');
    showToast('Error: ' + (err.message || 'Gagal menghubungi OpenRouter.'), 'error');
    console.error(err);
  }
}
